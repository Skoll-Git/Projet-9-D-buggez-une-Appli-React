...
const Page = () => {
  const { data } = useData();
  const last = data?.events?.[data.events.length - 1]; // Dernier élément du tableau events
      ...
      <main>
        ...
        <footer className="row">
          <div className="col presta">
            ...
            {last ? (
              <EventCard
                imageSrc={last.cover || "/images/default-image.png"}
                title={last.title || "Titre indisponible"}
                date={last.date ? new Date(last.date) : "Date inconnue"}
                small
                label="boom"
              />
            ) : (
